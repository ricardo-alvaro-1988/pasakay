...
public static class FareQuote
{
    ...
    public static decimal ComputeForPassengers(DeriveFareMatrix? matrix, int passengerCount, decimal distanceKm)
    {
        var count = Math.Max(1, passengerCount);
        if (matrix is null)
        {
    ...
    public static DeriveFarePassengerTier? ResolveTier(DeriveFareMatrix matrix, int passengerCount)
    {
        var count = Math.Max(1, passengerCount);
        var tiers = matrix.PassengerTiers;
        if (tiers is null || tiers.Count == 0)
    ...
    public static IReadOnlyList<FareSampleItem> SamplesForPassengers(DeriveFareMatrix matrix, int passengerCount) =>
        Enumerable.Range(1, 10)
            .Select(km => new FareSampleItem(km, ComputeForPassengers(matrix, passengerCount, km)))
            .ToList();
}
