...
class OnlineService : Service() {
        ...
        try {
            ...
            when (intent?.action) {
                ACTION_RING -> startRing(
                    ...
                    intent.getStringExtra("body") ?: "Open Pricebadz to accept.",
                )
                ACTION_CHAT -> showChat(
                    intent.getStringExtra("title") ?: "New chat",
                    intent.getStringExtra("body") ?: "Open Pricebadz to reply.",
                )
    ...
    private fun acquireWake() {
        try {
            ...
            wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "pricebadz:offer").apply {
