  ...
  return (
    <div className={`shell${collapsed ? ' collapsed' : ''}`}>
      ...
      <main className="main">
        ...
        {page === 'customer-report' && <OperatorCustomerReportPage />}
...
function OperatorBookingReportPage() {
  const today = isoDate(new Date())
  const [dateMode, setDateMode] = useState<'date' | 'range'>('date')
  const [from, setFrom] = useState(today)
  const [to, setTo] = useState(today)
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => {
      api.operatorBookingReport({
        ...
        .then(setData)
        .catch((err: Error) => setError(err.message))
    }, bookingNo || rider || customer ? 220 : 0)
    return () => window.clearTimeout(handle)
  }, [dateMode, fromParam, toParam, bookingNo, rider, customer, page])

  const rows = data?.page.items ?? []
  const total = data?.page.total ?? 0
  const pages = Math.max(1, Math.ceil(total / pageSize))

  function openBooking(row: BookingReportItem) {
    window.open(
      `${window.location.pathname}${window.location.search}#commission/operator/_/${row.id}`,
      '_blank',
      'noopener,noreferrer',
    )
  }
  ...
  async function exportExcel() {
    ...
    setError('')
    try {
      await api.exportOperatorBookingReport({
        bookingNo: bookingNo.trim() || undefined,
        rider: rider.trim() || undefined,
        customer: customer.trim() || undefined,
        from: fromParam,
      ...
      setError(err instanceof Error ? err.message : 'Could not export booking report.')
    } finally {
      setExportBusy(false)
    }
  }
      ...
      <div className="card">
        <div className="toolbar">
          <div>
            ...
            <p className="muted" style={{ margin: '4px 0 0' }}>
              Individual completed bookings with commission, promo, and fare for the selected date or range.
            </p>
          </div>
        </div>
        <div className="ride-filters" style={{ marginTop: 12 }}>
          ...
          <button className="btn tiny" type="button" disabled={exportBusy} onClick={() => void exportExcel()}>
        ...
        {data ? (
          <div className="fare-cards" style={{ marginTop: 14 }}>
            ...
            <div className="detail-card">
              <span>Rider comm</span>
              <p className="detail-name" style={{ marginTop: 8 }}>{peso(data.summary.riderCommission)}</p>
            </div>
            <div className="detail-card">
              <span>System comm</span>
              <p className="detail-name" style={{ marginTop: 8 }}>{peso(data.summary.systemCommission)}</p>
            </div>
            <div className="detail-card">
              <span>Operator comm</span>
              <p className="detail-name" style={{ marginTop: 8 }}>{peso(data.summary.operatorCommission)}</p>
            </div>
            <div className="detail-card">
              <span>Promo</span>
              <p className="detail-name" style={{ marginTop: 8 }}>{peso(data.summary.promo)}</p>
            </div>
            <div className="detail-card">
              <span>Fare</span>
              <p className="detail-name" style={{ marginTop: 8 }}>{peso(data.summary.fare)}</p>
            </div>
          </div>
        ) : null}
        <div className="table-wrap" style={{ marginTop: 16 }}>
          <table>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={9}>No bookings for this filter.</td></tr>
              ) : rows.map((row: BookingReportItem) => (
                <tr
                  key={row.id}
                  className="clickable"
                  onClick={() => openBooking(row)}
...
function OperatorRiderReportPage() {
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => {
      api.opRiders(q, 1, 8).then((rows) => setSuggest(rows.items)).catch(() => setSuggest([]))
    }, 200)
    return () => window.clearTimeout(handle)
  }, [q])
      ...
      <div className="card">
        ...
        {data ? (
          <div className="fare-cards" style={{ marginTop: 14 }}>
            ...
            <div className="detail-card">
              <span>No# cancel</span>
              <p className="detail-name" style={{ marginTop: 8 }}>{data.summary.totalCancel}</p>
            </div>
            <div className="detail-card">
        ...
        <div className="table-wrap" style={{ marginTop: 16 }}>
          <table>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={9}>No riders for this filter.</td></tr>
...
function OperatorCustomerReportPage() {
  const today = isoDate(new Date())
  const [dateMode, setDateMode] = useState<'date' | 'range'>('date')
  const [from, setFrom] = useState(today)
  const [to, setTo] = useState(today)
  const [q, setQ] = useState('')
  const [suggest, setSuggest] = useState<CustomerListItem[]>([])
  const [page, setPage] = useState(1)
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => {
      api.opCustomers(q).then(setSuggest).catch(() => setSuggest([]))
    }, 200)
    return () => window.clearTimeout(handle)
  }, [q])
  ...
  async function exportExcel() {
    ...
    setError('')
    try {
      await api.exportOperatorCustomerReport({
        q: q.trim() || undefined,
        from: fromParam,
        to: toParam,
      })
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Could not export customer report.')
    } finally {
      setExportBusy(false)
    }
  }
      ...
      <div className="card">
        <div className="toolbar">
          <div>
            ...
            <p className="muted" style={{ margin: '4px 0 0' }}>
              Customers with completed rides, cancels, and spend for the selected date or range.
            </p>
          </div>
        </div>
        <div className="ride-filters" style={{ marginTop: 12 }}>
        ...
        {data ? (
          <div className="fare-cards" style={{ marginTop: 14 }}>
            <div className="detail-card">
              <span>Customers</span>
              <p className="detail-name" style={{ marginTop: 8 }}>{data.summary.customerCount}</p>
            </div>
            <div className="detail-card">
              <span>No# rides</span>
              <p className="detail-name" style={{ marginTop: 8 }}>{data.summary.totalRides}</p>
            ...
            <div className="detail-card">
              ...
              <p className="detail-name" style={{ marginTop: 8 }}>{peso(data.summary.totalSpent)}</p>
            </div>
          </div>
        ) : null}
        <div className="table-wrap" style={{ marginTop: 16 }}>
          <table>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={7}>No customers for this filter.</td></tr>
              ) : rows.map((row: CustomerReportItem) => (
                <tr key={row.customerId}>
                  <td><strong>{row.customerName}</strong>{row.isActive ? null : <span className="muted"> · Off</span>}</td>
                  <td>{row.mobile || '—'}</td>
                  <td>{phDateTime(row.joinedAtUtc)}</td>
                  <td>{row.totalRides}</td>
                  <td>{row.totalCancel}</td>
                  <td>{peso(row.bookingAmount)}</td>
                  <td><strong>{peso(row.totalSpent)}</strong></td>
                </tr>
              ))}
            </tbody>
          </table>
